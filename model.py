...
class Autoencoder(nn.Module):
    ...
    def reconstruction_error_mse(self, x):
        x_hat, _ = self.forward(x)
        return torch.square(x[:, 400:1500] - x_hat[:, 400:1500]).mean(dim=1)
    
    def reconstruction_error_huber(self, x, soglia):
        x_hat, _ = self.forward(x)
        loss = torch.nn.HuberLoss(reduction='none', delta = soglia)
        return loss(x[:, 400:1500], x_hat[:, 400:1500]).mean(dim=1) 
    
    def reconstruction_error_pesata(self, x, std_norm):
        x_hat, _ = self.forward(x)
        alpha = 1

        residuo = (x[:, 400:1500] - x_hat[:, 400:1500]) * float(std_norm) 
        segnale = (x[:, 400:1500] * float(std_norm)).sum(dim=1)   
